 ...
 * @param ball_collision determines which collision detection to use
 */
...
/**
 * Adds a force creator that creates a frictional force between the body and
 * whatever surface it's on. This computes the frictional force that will be
 * applied to the body after each tick. The force acts opposite the body's
 * direction of motion. The frictional force is proportional to the mass of the
 * body.
 *
 * @param scene the scene containing the bodies
 * @param mug the coefficient of friction (mu) multiplied with the acceleration
 * due to gravity (g) to get a proportionality constant (mug) for the frictional
 * force
 * @param body the body that the frictional force is acting upon
 */
