...
void test_list_size1() {
    ...
    *v = VEC_ZERO;
    ...
    // Remove
    assert((vector_t *) list_remove(l, 0) == v);
    free(v);
    assert(list_size(l) == 0);
    // Add again
    v = malloc(sizeof(*v));
...
void test_list_large_add_remove() {
    list_t *l = list_init(MEDIUM_SIZE, (free_func_t)free);
    // Add to capacity
    for (size_t i = 0; i < LARGE_SIZE; i++) {
        vector_t *v = malloc(sizeof(*v));
        v->x = v->y = i;
    ...
    // Remove all
    for (size_t i = 1; i <= MEDIUM_SIZE; i++) {
        size_t value = MEDIUM_SIZE - i;
        vector_t *v = (vector_t *) list_remove(l, value);
...
void test_full_add() {
    ...
    list_add(l, malloc(sizeof(vector_t)));
    assert(list_size(l) == size+1);

    list_free(l);
}

void remove_from_empty(void *l) {
    list_remove((list_t *) l, 0);
}
